# DeployMind — CIPHER Task 5

A multi-agent deployment automation system built with Python, Google ADK 2.3.0, Gemini 2.5 Flash Lite, and Qwen3 via Ollama/LiteLLM. DeployMind automatically deploys a GitHub repository to Vercel, detects errors, classifies them, suggests fixes, validates the fixes, and opens a GitHub Pull Request for human review — all coordinated by a central orchestrator.
...
**Model:** Gemini 2.5 Flash Lite + `google_search`  
...
| Agent | Model | Reason |
|---|---|---|
| Deployment Agent | Qwen3 (Ollama) | Tool calls only (Vercel REST API), no LLM reasoning needed |
| Log Analysis Agent | Qwen3 (Ollama) | Pure reasoning, no external tools |
| Issue Classification Agent | Qwen3 (Ollama) | Pure reasoning, fixed enum output |
| Fix Research Agent | Gemini 2.5 Flash Lite | Requires `google_search` built-in tool |
| Fix Suggestion Agent | Gemini 2.5 Flash Lite | Structures research output via `output_schema` |
| Validation Agent | Qwen3 (Ollama) | Pure reasoning, no external tools |
| Approval Agent | Qwen3 (Ollama) | Pure reasoning, no external tools |
| Patch Generation Agent | Gemini 2.5 Flash Lite | Requires GitHub API tool calls |
...
#   d e p l o y m e n t _ a g e n t  
 